    ...
    if ( $post instanceof  WP_Post ) {
        ...
        if ( !empty( $tdb_template_id ) ) {
            ...
            if ( ! empty( $wp_query_template ) && $wp_query_template->have_posts() ) {
                td_get_template_style( $wp_query_template->post );
            }
        }
    }
});
...
function td_get_template_style( $template = null, $show_style = true ) {
        ...
        foreach ( $content_matches[1] as $str_atts ) {
            ...
            if ( count( $style_matches ) && is_array( $style_matches[0] ) ) {
                foreach( $style_matches[0] as $style_match ) {
                    // find inline css
                    $style .= preg_replace('/.td_uid_(\X*)_rand/miU', '.td-gutenberg-editor .editor-styles-wrapper .wp-block', $style_match);
                }
            }
